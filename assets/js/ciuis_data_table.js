...
function Ciuis_Controller(
  ...
  $http.get(BASE_URL + "api/user").then(function (Userman) {
    ...
    $http
      .get(BASE_URL + "api/lang/" + $scope.user.language)
    ...
    $scope.ChangeAppointmentAvailability = function (id, value) {
      $http
        .post(BASE_URL + "staff/appointment_availability/" + id + "/" + value)
        .then(
          function (response) {
  ...
  $scope.search_customers = function (q) {
    if (q.length > 0) {
      $http
        .get(BASE_URL + "api/search_customers/" + q)
  ...
  globals.deleteDialog = function (
    ...
    $mdDialog.show(confirm).then(
      function () {
        $http.post(BASE_URL + api, config).then(
          function (response) {
  ...
  globals.editDialog = function (
    ...
    $mdDialog.show(confirm).then(
      function (result) {
        ...
        $http.post(BASE_URL + api, dataObj, config).then(
          function (response) {
            callback(response.data);
          },
          function (response) {
  ...
  globals.mdToast = function (type, message, time = 4000) {
    $mdToast.show({
      template:
        '<md-toast class="md-toast ' + type + '">' + message + "</md-toast>",
      hideDelay: time,
      position: "top center",
    });
  };
  ...
  $scope.DeleteEventType = function (id) {
    var confirm = $mdDialog
      .confirm()
      .title(
        $scope.lang.delete + " " + $scope.lang.event + " " + $scope.lang.type
      )
    ...
    $mdDialog.show(confirm).then(
      function () {
        $http.post(BASE_URL + "api/remove_eventtype/" + id, config).then(
          function (response) {
            getEventTypes();
            showToast(NTFTITLE, response.data.message, " success");
          },
      ...
      $http.post(posturl, dataObj, config).then(
        function (response) {
          if (response.data.success == true) {
            ...
            $http
              .get(BASE_URL + "api/settings_detail")
  ...
  $scope.DeleteMenuTimer = function (id) {
    ...
    $mdDialog.show(confirm).then(
      function () {
        $http.post(BASE_URL + "api/delete_timer/" + id, config).then(
          function (response) {
            getMenuTimer();
            globals.mdToast("success", response.data.message);
            $http
  ...
  $scope.stopTimerWithTask = function (action, id) {
    ...
    function NewTeamDialogController($scope, $mdDialog) {
      $http
        .post(BASE_URL + "api/get_timer_data/" + id, config)
  ...
  $scope.TodoAsUnDone = function (index) {
    ...
    $http.post(BASE_URL + "trivia/undonetodo", dataObj, config).then(
      function (response) {
        var todo = $scope.tododone[index];
  ...
  $scope.TodoAsDone = function (index) {
    ...
    $http.post(BASE_URL + "trivia/donetodo", dataObj, config).then(
      function (response) {
        var todo = $scope.todos[index];
        $scope.todos.splice($scope.todos.indexOf(todo), 1);
  ...
  $scope.ChangeTicketStatus = function () {
    ...
    $http.post(BASE_URL + "tickets/chancestatus", dataObj, config).then(
      function (response) {
        $.gritter.add({
          title: "<b>" + NTFTITLE + "</b>",
  ...
  $scope.createPassword = function () {
    ...
    var noOfLowerCharacters = 0,
      ...
      noOfSymbols = 0;
    var noOfneededTypes =
      $scope.addUpper + $scope.addNumbers + $scope.addSymbols;
    var noOfLowerCharacters = getRandomInt(
      1,
      $scope.passwordLength - noOfneededTypes
    );
    var usedTypeCounter = 1;
    if ($scope.addUpper) {
      noOfUpperCharacters = getRandomInt(
        1,
    ...
    for (var i = 0; i < noOfLowerCharacters; i++) {
      passwordArray.push(
        lowerCharacters[getRandomInt(1, lowerCharacters.length - 1)]
      );
    }
    for (var i = 0; i < noOfUpperCharacters; i++) {
      passwordArray.push(
        upperCharacters[getRandomInt(1, upperCharacters.length - 1)]
      );
    }
    ...
    Highcharts.chart(
      "monthlyexpenses",
      {
        ...
        tooltip: {
          ...
          formatter: function () {
            return (
              '<div class="bis-tooltip" style="background-color: ' +
              this.color +
              '">' +
              this.x +
              " <span>" +
...
CiuisCRM.directive("loadMore", function () {
  ...
  return {
    template:
      "<a ng-click='loadMore()' id='loadButton' class='activity_tumu'><i style='font-size:22px;' class='icon ion-android-arrow-down'></i></a>",
    link: function (scope) {
      scope.LogLimit = 2;
    ...
    link: function ($scope) {
      setTimeout(function () {
        $scope.$apply(function () {
          var milestone_projectExpandablemilestonetitle = $(
            ".milestone_project-action.is-expandable .milestonetitle"
          );
          ...
          $(".milestone_project").each(function (i, $milestone_project) {
            var $milestone_projectActions = $($milestone_project).find(
              ".milestone_project-action.is-expandable"
            );
            $($milestone_projectActions).each(function (
              j,
              $milestone_projectAction
            ) {
              var $milestoneContent = $($milestone_projectAction).find(
                ".content"
              );
              $($milestoneContent)
                .attr(
                  "id",
                  "milestone_project-" + i + "-milestone-content-" + j
                )
                .attr("role", "region");
              $($milestoneContent).attr(
                "aria-expanded",
                $($milestone_projectAction).hasClass("expanded")
              );
              $($milestone_projectAction)
                .find(".milestonetitle")
                .attr(
          ...
          $(milestone_projectExpandablemilestonetitle).click(function () {
            ...
            $(this)
              .siblings(".content")
              .attr("aria-expanded", $(this).parent().hasClass("is-expanded"));
          });
          ...
          $(milestone_projectExpandablemilestonetitle).keyup(function (e) {
            if (e.which === 13) {
              ...
              // Left or Up
              $(this)
                .closest(".milestone_project-milestone")
                .prev(".milestone_project-milestone")
                .find(".milestone_project-action .milestonetitle")
                .focus();
            } else if (e.which === 39 || e.which === 40) {
              // Right or Down
              $(this)
                .closest(".milestone_project-milestone")
                .next(".milestone_project-milestone")
                .find(".milestone_project-action .milestonetitle")
                .focus();
            }
          });
  ...
  return {
    ...
    link: function (scope, element, attrs, ngModelController) {
      ngModelController.$parsers.push(function (data) {
        ...
        return (
          ("0" + data.getHours().toString()).slice(-2) +
          ":" +
          ("0" + data.getMinutes().toString()).slice(-2)
        );
      });
